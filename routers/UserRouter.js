...
router.get("/follow/:userid", auth, async (req, res) => {
  try {
    ...
    const clickedUser = await User.findByIdAndUpdate(
      { _id: req.params.userid },
      { $push: { followers: putToClickedUser } }
    );
    ...
    const loggedInUser = await User.findByIdAndUpdate(
      { _id: req.user._id },
      { $push: { followings: putToLoggedInUser } }
    );
    ...
    gfs.files.find().toArray(async (err, files) => {
      if (!files || files.length === 0) {
        ...
        const updateUser = await User.findByIdAndUpdate(
          { _id: req.params.userid },
          { profilepicture: filename, profilepicturestatus: true }
        );
